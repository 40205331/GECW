...
void EnemyTurretComponent2::update(double dt) {
	_firetime -= dt;
	if (_firetime <= 0.f) {
		fire();
		_firetime = 1.f;
	...
	_parent->setRotation(-90.f + sin(angle) * 45.f);
}

void EnemyTurretComponent2::fire() const {
...
EnemyTurretComponent2::EnemyTurretComponent2(Entity* p)
